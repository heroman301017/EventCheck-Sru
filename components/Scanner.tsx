...
export const Scanner: React.FC<ScannerProps> = ({ users, onScan }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center max-w-2xl mx-auto p-6 space-y-8 animate-fade-in">
      
      {/* Settings Toggle */}
      <div className="w-full flex justify-end">
        <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer bg-white px-3 py-1 rounded-full shadow-sm">
          <input 
            type="checkbox" 
            checked={autoSave} 
      ...
      <div className="relative w-full max-w-md bg-gray-900 rounded-3xl aspect-[4/3] flex flex-col items-center justify-center overflow-hidden shadow-2xl border-4 border-gray-800">
        {lastScanResult.status === 'idle' && (
          <>
            ...
            <p className="mt-4 text-blue-200 font-medium">Ready to Scan QR Code</p>
