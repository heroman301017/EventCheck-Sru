...
import { Plus, Edit3, Trash2, Save, X, Calendar, MapPin, AlignLeft, Eye, EyeOff } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {events.map(event => (
          <div key={event.id} className={`p-6 rounded-3xl border shadow-sm flex justify-between items-start hover:shadow-md transition-all ${event.isActive ? 'bg-white border-slate-100' : 'bg-slate-50 border-slate-200 opacity-75'}`}>
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                 <h3 className="font-bold text-slate-800 text-lg">{event.name}</h3>
                 {!event.isActive && <span className="px-2 py-0.5 bg-slate-200 text-slate-500 text-[10px] rounded-full font-bold">Hidden</span>}
              </div>
            ...
            <div className="flex gap-2 shrink-0 ml-4">
              <button 
                ...
                className={`p-2 rounded-lg transition-colors ${event.isActive ? 'text-emerald-500 hover:bg-emerald-50' : 'text-slate-400 hover:bg-slate-200'}`}
                title={event.isActive ? "ซ่อนกิจกรรม" : "แสดงกิจกรรม"}
              >
                {event.isActive ? <Eye className="w-5 h-5" /> : <EyeOff className="w-5 h-5" />}
              </button>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 max-w-lg w-full shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-2 py-2">
                 ...
                 />
                 <label htmlFor="isActive" className="text-sm font-bold text-slate-600">แสดงกิจกรรมนี้ในหน้าลงทะเบียน (Active)</label>
              </div>
